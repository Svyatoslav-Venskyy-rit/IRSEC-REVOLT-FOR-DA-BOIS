<#
.SYNOPSIS
  Compare current local & domain accounts to an allowed list, prompt to delete any extras, and log everything to CSV.

.USAGE
  ./cleanup_accounts.ps1
  ./cleanup_accounts.ps1 -IncludeDisabled

.NOTES
  - **ELEVATION IS REQUIRED:** Run as Elevated Administrator for local deletions.
...
# Define built-in local accounts to explicitly avoid deleting
$builtInLocalAccounts = @("administrator", "guest", "defaultaccount", "wdagutilityaccount")
...
# **FIXED:** Use a more robust way to get the full operator name for the IR report
...
function Add-Log {
    ...
    $log += [PSCustomObject]@{
        ...
        Note        = $Note
        PerformedBy = $currentUser   # The confirmed domain\user or machine\user
    }
}
...
if ($haveADModule) {
    try {
        ...
        # Fetch users (filter out built-in/service accounts starting with $)
    ...
    try {
        ...
        $searcher.PropertiesToLoad.Add("userAccountControl") | Out-Null # Needed for enabled status

        if ($IncludeDisabled) {
            $searcher.Filter = "(&(objectCategory=person)(objectClass=user)(!(sAMAccountName=*$)))" # Filter out service accounts
        } else {
            # Filter for enabled users and filter out service accounts
        ...
        foreach ($r in $results) {
            ...
            if ($name) {
                # Determine enabled status based on UAC flag 2 (ACCOUNTDISABLE)
...
# --- Evaluate allowed vs discovered ---
# Domain extras
$domainExtras = @()
foreach ($u in $domainUsers) {
    ...
    if (-not ($allowedDomain -contains $uname)) {
        $domainExtras += $u
    }
}
...
foreach ($u in $localUsers) {
    ...
    # **FIXED:** Exclude allowed accounts AND built-in accounts
    if (-not ($allowedLocal -contains $uname) -and -not ($builtInLocalAccounts -contains $uname)) {
        $localExtras += $u
    }
}
...
# --- Finalize log ---
try {
    # Using UTF8 for broader compatibility with different characters, if needed
