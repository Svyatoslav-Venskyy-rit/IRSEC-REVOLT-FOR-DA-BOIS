...
$logFile = (Resolve-Path (Join-Path $logDir "anomalies_$timestamp.log")).Path  # Absolute path for emails
...
function Get-Whitelist {
    ...
    switch ($hostname) {
        "Wright Brothers" { $hostSpecificServices["LanmanServer"] = $true } # SMB (Server service)
        "Moon Landing" { $hostSpecificServices["W3SVC"] = $true }         # IIS (World Wide Web Publishing)
        "Pyramids" { 
